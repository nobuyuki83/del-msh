    ...
    uint32_t* vtx2morton)
{
  ...
  // 
  if (num_dim == 2 ) {
    const float* xy0 = vtx2xyz + i_vtx * 2;
    auto xy1 = mat3_col_major::transform_homogeneous(transform_cntr2uni, xy0);
    assert(xy1[0]>-1.0e-7f && xy1[0]<1.0f+1.e-7f);
    assert(xy1[1]>-1.0e-7f && xy1[1]<1.0f+1.e-7f);
    uint32_t mc = device_morton_code2(xy1[0], xy1[1]);
    vtx2morton[i_vtx] = mc;
  }
  else if( num_dim == 3 ) {
    ...
    uint32_t mc = device_morton_code3(xyz1[0], xyz1[1], xyz1[2]);
    vtx2morton[i_vtx] = mc;
  }
}
