...
fn io_vtk_write_mix_mesh(
    ...
    let hex2vtx = del_dlpack::get_managed_tensor_from_pyany(hex2vtx)?;
    //
    ...
    let num_hex = del_dlpack::get_shape_tensor(hex2vtx, 0).unwrap();
    //
    ...
    del_dlpack::check_2d_tensor::<u32>(prism2vtx, num_prism, 6, dlpack::device_type_codes::CPU)
        .unwrap();
    del_dlpack::check_2d_tensor::<u32>(hex2vtx, num_hex, 8, dlpack::device_type_codes::CPU)
        .unwrap();
    //
    ...
    let hex2vtx = unsafe { del_dlpack::slice_from_tensor::<u32>(hex2vtx) }.unwrap();
    //
