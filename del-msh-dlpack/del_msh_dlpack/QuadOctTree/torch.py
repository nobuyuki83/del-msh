...
def bnodes_and_bnode2depth_and_bnode2onode_and_idx2bnode(
    idx2morton: torch.Tensor, num_dim, max_depth: int | None = None
):
    #
...
class QuadOctTree:
    ...
    def construct_from_idx2morton(
        self, idx2morton: torch.Tensor, num_dim: int, is_save_intermediate=False
    ):
        self.num_dim = num_dim
        (bnodes, bnode2depth, bnode2onode, idx2bnode) = (
            bnodes_and_bnode2depth_and_bnode2onode_and_idx2bnode(idx2morton, num_dim)
        )
        (
            self.onodes,
            self.onode2depth,
            ...
            bnodes, bnode2onode, bnode2depth, idx2bnode, idx2morton, num_dim
        )
    ...
    def aggregate(self, idx2val) -> torch.Tensor:
        ...
        #
        assert (
            idx2val.shape == (num_idx, num_vdim)
            and idx2val.device == device
            and idx2val.dtype == torch.float32
        )
        #
        onode2aggval = torch.zeros(
            size=(num_onode, num_vdim), dtype=torch.float32, device=device
        )
        #
        stream_ptr = 0
        if device.type == "cuda":
