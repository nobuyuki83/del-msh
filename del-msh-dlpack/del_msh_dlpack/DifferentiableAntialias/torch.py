...
def antialias(
    ...
    util_torch.assert_shape_dtype_device(
        cedge2vtx, (num_contour, 2), torch.uint32, device
    )
    util_torch.assert_shape_dtype_device(vtx2xyz, (num_vtx, 3), torch.float32, device)
    util_torch.assert_shape_dtype_device(
        transform_world2pix, (4, 4), torch.float32, device
    )
...
def bwd_antialias(
    ...
    util_torch.assert_shape_dtype_device(
        dldw_pix2val, (img_h, img_w), torch.float32, device
    )
...
class DifferentiableAntialiasFunction(torch.autograd.Function):
    ...
    def backward(ctx, dldw_pix2vout):
        ...
        dldw_vtx2xyz = bwd_antialias(
            cedge2vtx, vtx2xyz, transform_world2pix, pix2vin, dldw_pix2vout, pix2tri
        )
