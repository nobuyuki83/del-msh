...
def write_velocity_on_staggered_grid(
    path: str, hedge2vy: torch.Tensor, vedge2vx: torch.Tensor
):
...
def write_points_with_velocity(
    path: str, vtx2xyz: torch.Tensor, vtx2velocity: torch.Tensor
):
...
def write_mix_mesh(
    ...
    util_torch.assert_shape_dtype_device(
        vtx2xyz, (num_vtx, 3), torch.float32, torch.device("cpu")
    )
    util_torch.assert_shape_dtype_device(
        tet2vtx, (num_tet, 4), torch.uint32, torch.device("cpu")
    )
    util_torch.assert_shape_dtype_device(
        pyrmd2vtx, (num_pyrmd, 5), torch.uint32, torch.device("cpu")
    )
    util_torch.assert_shape_dtype_device(
        prism2vtx, (num_prism, 6), torch.uint32, torch.device("cpu")
    )
    util_torch.assert_shape_dtype_device(
        hex2vtx, (num_hex, 8), torch.uint32, torch.device("cpu")
    )
    #
    from .. import IoVtk
