...
def vtx2morton_from_vtx2co(
    vtx2co: np.ndarray, transform_co2unit: np.ndarray
) -> np.ndarray:
    ...
    util_numpy.assert_shape_dtype(
        transform_co2unit, shape=(num_dim + 1, num_dim + 1), dtype=np.float32
    )
    #
...
def make_bvhnodes_from_sorted_mortons(
    idx2obj: np.ndarray, idx2morton: np.ndarray
) -> np.ndarray:
    n = idx2obj.shape[0]
    #
