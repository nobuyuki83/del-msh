...
class TreeAccelerator:
    ...
    def initialize(self, vtx2xyz: torch.Tensor):
        ...
        jdx2idx, idx2morton, self.idx2jdx_offset = (
        ...
        self.onode2cgunit = self.construct_center_of_gravity(
            self.idx2jdx_offset, self.jdx2vtx, vtx2xyz, self.transform_world2unit
        )
        self.vtx2xyz = vtx2xyz

    def construct_center_of_gravity(
        self, idx2jdx_offset, jdx2vtx, vtx2xyz, transform_world2unit
    ):
        ...
        idx2aggxyz = del_msh_dlpack.OffsetArray.torch.aggregate(
            idx2jdx_offset, jdx2vtx, vtx2xyz
        )
        ...
        ones = torch.ones(
            (onode2cgxyz.shape[0], 1), dtype=torch.float, device=onode2cgxyz.device
        )
...
def filter_with_acceleration(
    ...
    util_torch.assert_shape_dtype_device(
        acc.vtx2xyz.detach(), (num_vtx, 3), torch.float32, device
    )
    util_torch.assert_shape_dtype_device(acc.jdx2vtx, (num_vtx,), torch.uint32, device)
    util_torch.assert_shape_dtype_device(
        acc.tree2idx.idx2onode, (num_idx,), torch.uint32, device
    )
    util_torch.assert_shape_dtype_device(
        acc.tree2idx.onodes, (num_onode, 9), torch.uint32, device
    )
    util_torch.assert_shape_dtype_device(
        acc.tree2idx.onode2depth, (num_onode,), torch.uint32, device
    )
    util_torch.assert_shape_dtype_device(
        acc.tree2idx.onode2center, (num_onode, 3), torch.float32, device
    )
    #
    idx2aggrhs = del_msh_dlpack.OffsetArray.torch.aggregate(
        acc.idx2jdx_offset, acc.jdx2vtx, vtx2rhs
    )
    ...
    NBody.filter_with_acceleration(
        ...
        util_torch.to_dlpack_safe(
            acc.transform_world2unit.permute(1, 0).contiguous().view(-1), stream_ptr
        ),
