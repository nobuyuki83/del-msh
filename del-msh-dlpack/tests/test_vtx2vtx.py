...
def test_01():
    ...
    vtx2vtx = del_msh_dlpack.Vtx2Vtx.np.from_uniform_mesh(
    ...
    residual = numpy.linalg.norm(
        vtx2lhs
        + lambda0
        * del_msh_dlpack.Vtx2Vtx.np.multiply_graph_laplacian(*vtx2vtx, vtx2lhs)
        - vtx2rhs
    )
...
def test_02():
    ...
    vtx2vtx = del_msh_dlpack.Vtx2Vtx.pt.from_uniform_mesh(
        tri2vtx, vtx2xyz.shape[0], False
    )
    ...
    residual = numpy.linalg.norm(
        vtx2lhs
        + lambda0
        * del_msh_dlpack.Vtx2Vtx.pt.multiply_graph_laplacian(*vtx2vtx, vtx2lhs)
        - vtx2rhs
    )
    assert residual < 3.0e-5
    #
...
def test_03():
    ...
    vtx2xyz = torch.from_numpy(vtx2xyz)
    #
    h_vtx2vtx = del_msh_dlpack.Vtx2Vtx.pt.from_uniform_mesh(
        tri2vtx, vtx2xyz.shape[0], False
    )
    if torch.cuda.is_available():
        d_vtx2vtx = del_msh_dlpack.Vtx2Vtx.pt.from_uniform_mesh(
