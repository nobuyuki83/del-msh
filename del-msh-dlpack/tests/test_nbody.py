...
def test_spoisson_3d():
    ...
    theta = 0.2
    wtx2lhs0 = del_msh_dlpack.NBody.torch.filter_brute_force(
        vtx2xyz, vtx2rhs, model, wtx2xyz
    )
    #
    acc = del_msh_dlpack.NBody.torch.TreeAccelerator()
    acc.initialize(vtx2xyz)
    wtx2lhs1 = del_msh_dlpack.NBody.torch.filter_with_acceleration(
        vtx2rhs, model, wtx2xyz, acc, theta
    )
    ...
    if torch.cuda.is_available():
        ...
        d_wtx2lhs1 = del_msh_dlpack.NBody.torch.filter_with_acceleration(
            vtx2rhs.cuda(), model, wtx2xyz.cuda(), d_acc, theta
        )
