...
def test_01():
    ...
    #
    ptcpu_tri2normal = del_msh_dlpack.TriMesh3.torch.make_tri2normal(
        ptcpu_tri2vtx, ptcpu_vtx2xyz
    )
...
def test_03():
    ...
    for i_vtx in range(0, num_vtx):
        for i_dim in range(0, 3):
            ...
            tri2nrm1 = del_msh_dlpack.TriMesh3.numpy.tri2normal(tri2vtx, vtx2xyz1)
...
def test_04():
    ...
    optimizer.zero_grad()
    tri2nrm = del_msh_dlpack.TriMesh3.torch.Tri2Normal.apply(tri2vtx, vtx2xyz)
    ...
    if torch.cuda.is_available():
        ...
        optimizer = torch.optim.Adam([vtx2xyz], lr=0.001)
        optimizer.zero_grad()
        tri2nrm = del_msh_dlpack.TriMesh3.torch.Tri2Normal.apply(
            tri2vtx.cuda(), vtx2xyz
        )
