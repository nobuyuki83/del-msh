...
def test_unique_for_sorted_array():
    ...
    idx2jdx, jdx2val, jdx2idx_offset = (
        del_msh_dlpack.Array1D.torch.unique_for_sorted_array(idx2val)
    )
    assert torch.equal(idx2jdx, torch.tensor([0, 1, 2, 3, 3, 4], dtype=torch.uint32))
    if torch.cuda.is_available():
        d_idx2jdx, d_jdx2val, d_jdx2idx_offset = (
            del_msh_dlpack.Array1D.torch.unique_for_sorted_array(idx2val.cuda())
        )
...
def test_hoge():
    x0 = torch.full((128, 128), 0)
    x1 = torch.full((128, 128), -1, dtype=torch.int32)
    x2 = torch.full(
        (128, 128),
        -1,
        dtype=torch.int32,
