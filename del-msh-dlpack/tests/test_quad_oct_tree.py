...
def test_tree_construction():
    for num_dim in range(2, 4):
        ...
        idx2jdx, jdx2morton, jdx2idx_offset = (
            del_msh_dlpack.Array1D.torch.unique_for_sorted_array(idx2morton)
        )
        assert not del_msh_dlpack.Array1D.torch.has_duplicate_in_sorted_array(
            jdx2morton
        )
        assert torch.equal(
            jdx2morton, torch.unique(idx2morton.to(torch.int64)).to(torch.uint32)
        )
        #
        ...
        if torch.cuda.is_available():
            ...
            (d_idx2vtx, d_idx2morton) = del_msh_dlpack.Array1D.torch.argsort(
                vtx2morton.cuda()
            )
            d_idx2jdx, d_jdx2morton, d_jdx2idx_offset = (
                del_msh_dlpack.Array1D.torch.unique_for_sorted_array(d_idx2morton)
            )
            assert not del_msh_dlpack.Array1D.torch.has_duplicate_in_sorted_array(
                d_jdx2morton
            )
            ...
            assert torch.equal(
                d_jdx2morton,
                torch.unique(d_idx2morton.to(torch.int64)).to(torch.uint32),
            )
...
def test_tree_aggregation():
    ...
    jdx2idx, idx2morton, idx2jdx_offset = (
        del_msh_dlpack.Array1D.torch.unique_for_sorted_array(jdx2morton)
    )
    assert not del_msh_dlpack.Array1D.torch.has_duplicate_in_sorted_array(idx2morton)
    assert torch.equal(
        idx2morton, torch.unique(jdx2morton.to(torch.int32)).to(torch.uint32)
    )
    #
    tree2idx = del_msh_dlpack.QuadOctTree.torch.QuadOctTree()
    tree2idx.construct_from_idx2morton(idx2morton, num_dim, False)
    #
    idx2aggval = del_msh_dlpack.OffsetArray.torch.aggregate(
        idx2jdx_offset, jdx2vtx, vtx2val
    )
    ...
    if torch.cuda.is_available():
        ...
        d_jdx2idx, d_idx2morton, d_idx2jdx_offset = (
            del_msh_dlpack.Array1D.torch.unique_for_sorted_array(d_jdx2morton)
        )
        ...
        assert torch.equal(
            d_idx2morton, torch.unique(d_jdx2morton.to(torch.int32)).to(torch.uint32)
        )
        ...
        d_idx2aggval = del_msh_dlpack.OffsetArray.torch.aggregate(
            d_idx2jdx_offset, d_jdx2vtx, vtx2val.cuda()
        )
