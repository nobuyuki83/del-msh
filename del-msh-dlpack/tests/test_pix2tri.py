...
def generate_lighting(low: float, mid: float, high: float, device) -> Tuple[torch.Tensor, torch.Tensor]:
...
def render_lambertian_shading_gouraud(
    ...
    pix2rgb_bg = torch.zeros((pix2tri.shape[0], pix2tri.shape[1],3), dtype=torch.float32)
    ...
    pix2diffuse = Pix2Tri.interpolate(pix2tri, tri2vtx, vtx2xyz, vtx2diffuse, transform_ndc2world)
    pix2rgb = pix2diffuse
    pix2rgb = torch.where(
...
def render_lambertian_shading_phong(
    ...
    background_pix = pix2tri == torch.iinfo(torch.uint32).max
    pix2rgb_bg = torch.zeros((pix2tri.shape[0], pix2tri.shape[1],3), dtype=torch.float32)
    pix2nrm = Pix2Tri.interpolate(pix2tri, tri2vtx, vtx2xyz, vtx2nrm, transform_ndc2world)
    pix2nrm_bg = pix2nrm.new_tensor([0.0, 0.0, 1.0])
    pix2nrm = torch.where(
        background_pix.unsqueeze(-1),
        pix2nrm_bg,
        pix2nrm,
...
def test_lambertian_shading():
    ...
    pix2rgb = render_lambertian_shading_phong(tri2vtx, vtx2xyz, vtx2nrm, transform_ndc2world, [0.0, 0.0, 1.0],
                                              [0.8, 1.0, 0.9], pix2tri)
    #
