...
def example2():
    ...
    pix2occ_target = torch.where(dist2 <= radius ** 2, 1.0, 0.0).to(torch.float32).unsqueeze(-1)
...
def test_smooth_gradient_staggered_grid():
    ...
    pix2tri = Pix2Tri.by_raycasting(tri2vtx, vtx2xyz, bvhnodes, bvhnode2aabb, transform_ndc2world, img_shape)
    #
    pix2occ_src = torch.where(pix2tri == torch.iinfo(torch.uint32).max, 0.0, 1.0).to(torch.float32).unsqueeze(-1)
...
def test_silhouette_optimization():
    ...
    for iter in range(0, 100):
        ...
        bvhnodes, bvhnode2aabb = TriMesh3.make_bvhnodes_bvhnode2aabb(tri2vtx, vtx2xyz)
        pix2tri = Pix2Tri.by_raycasting(tri2vtx, vtx2xyz, bvhnodes, bvhnode2aabb, transform_ndc2world, img_shape)
        pix2occ = torch.where(pix2tri == torch.iinfo(torch.uint32).max, 0.0, 1.0).to(torch.float32).unsqueeze(-1)
        #print(pix2occ.shape)
...
def example1(L_dir, L_color):
    ...
    transform1 = Mat44.from_translation(0., 0, 0.)
    ...
    pix2rgb_target = render_lambertian_shading_gouraud(
        tri2vtx, vtx2xyz, vtx2nrm, transform_ndc2world,
        L_dir, L_color, pix2tri)
...
def test_shading_optimization():
    ...
    tri2vtx, vtx2xyz, transform_world2ndc, img_shape, pix2rgb_trg = example1(L_dirs, L_colors)

    img = (pix2rgb_trg.detach().numpy() * 255).clip(0, 255).astype('uint8')
    path0 = pathlib.Path(__file__).parent.parent.parent / "target" / "del_msh_dlpack__microedge6.png"
    path0.parent.mkdir(exist_ok=True)
    Image.fromarray(img).save(path0)
    ...
    for iter in range(0, 30):
        ...
        path0 = pathlib.Path(__file__).parent.parent.parent / "target" / "del_msh_dlpack__microedge9.obj"
        TriMesh3.save_wavefront_obj(tri2vtx, vtx2xyz, str(path0))
