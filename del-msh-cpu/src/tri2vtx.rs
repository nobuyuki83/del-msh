...
pub fn from_polygon_mesh(elem2idx: &[usize], idx2vtx: &[usize]) -> (Vec<[usize; 3]>, Vec<usize>) {
    ...
    let mut tri2vtx = Vec::<[usize; 3]>::with_capacity(num_tri);
...
pub fn from_grid(nx: usize, ny: usize) -> Vec<[usize; 3]> {
    let mx = nx - 1;
    let my = ny - 1;
    let ncell = mx * my;
    let mut tri2vtx = Vec::<[usize; 3]>::with_capacity(ncell * 2);
    for imy in 0..my {
        for imx in 0..mx {
            let ivx0 = imx + imy * nx;
            ...
            tri2vtx.push([ivx0, ivx1, ivx2]);
            tri2vtx.push([ivx0, ivx2, ivx3]);
        }
    }
    assert_eq!(tri2vtx.len(), ncell * 2);
    tri2vtx
}
...
pub fn find_node_tri(tri2vtx: &[usize; 3], i_vtx: usize) -> usize {
    if tri2vtx[0] == i_vtx {
        return 0;
    }
    if tri2vtx[1] == i_vtx {
...
pub fn elem2elem(
    ...
    let (face2idx, idx2node) = if is_simplex_indexing {
        (
            del_geo_core::tri::FACE2IDX.to_vec(),
            del_geo_core::tri::IDX2NODE.to_vec(),
        )
    } else {
