...
{
    let mut aabb = [vtx2xy[0][0], vtx2xy[0][1], vtx2xy[0][0], vtx2xy[0][1]];
    vtx2xy.iter().skip(1).for_each(|v| {
...
pub fn aabb2_indexed<Index, T>(idx2vtx: &[Index], vtx2xy: &[[T; 2]], eps: T) -> [T; 4]
where
    T: num_traits::Float,
    Index: AsPrimitive<usize>,
{
    ...
    let mut aabb = [T::zero(); 4];
    {
        ...
        aabb[0] = vtx2xy[i_vtx][0] - eps;
        aabb[2] = vtx2xy[i_vtx][0] + eps;
        aabb[1] = vtx2xy[i_vtx][1] - eps;
        aabb[3] = vtx2xy[i_vtx][1] + eps;
    }
    for &i_vtx in idx2vtx.iter().skip(1) {
        let i_vtx = i_vtx.as_();
        let cgx = vtx2xy[i_vtx][0];
