    ...
    for node2vtx in cedge2vtx.chunks(2) {
        ...
        for &i_pix in list_pix.iter() {
            ...
            for (i_pix0, c0, i_pix1, c1) in hoge {
                ...
                // pix1 is background, pix0 is foreground
                ...
                if rc < 0.5 {
                    let r0 = 0.5 + rc;
                    pix2vout[i_pix0] = r0 * pix2vin[i_pix0] + (1.0 - r0) * pix2vin[i_pix1];
                } else {
                    let r0 = rc - 0.5;
                    pix2vout[i_pix1] = r0 * pix2vin[i_pix0] + (1.0 - r0) * pix2vin[i_pix1];
                }
            }
        }
    }
    ...
    for node2vtx in cedge2vtx.chunks(2) {
        ...
        for &i_pix in list_pix.iter() {
            ...
            for (i_pix0, c0, i_pix1, c1) in hoge {
                ...
                // ---------------------------------
                assert!((0. ..=1.0).contains(&rc));
                let dv = pix2val[i_pix0] - pix2val[i_pix1];
                let dldr0 = if rc < 0.5 {
                    dldw_pix2val[i_pix0] * dv
                } else {
                    dldw_pix2val[i_pix1] * dv
                };
