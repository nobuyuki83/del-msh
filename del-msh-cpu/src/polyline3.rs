...
fn test_vtx2framex() {
    let num_vtx = 10usize;
    let length = 3.0f64;
    let vtx2xyz: Vec<[f64; 3]> = (0..num_vtx)
    ...
    T: num_traits::Float + Copy,
{
    ...
    if i_vtx == num_vtx - 1 {
        let p0 = &vtx2xyz[num_vtx - 2];
        let p1 = &vtx2xyz[num_vtx - 1];
        return p1.sub(p0).normalize();
    }
    let p0 = &vtx2xyz[i_vtx - 1];
    let p1 = &vtx2xyz[i_vtx];
    let p2 = &vtx2xyz[i_vtx + 1];
    ...
    usize: num_traits::AsPrimitive<T>,
{
    ...
    for i_vtxl in 0..num_vtxl {
        ...
        for ic in 0..ndiv_circum {
            ...
            vtx2xyz[i_vtxl * ndiv_circum + ic] = q;
        }
    }
}
    ...
    for i_poly in 0..num_poly {
        for j_poly in i_poly + 1..num_poly {
            for i_seg in poly2vtx[i_poly]..poly2vtx[i_poly + 1] - 1 {
                let pi = &vtx2xyz[i_seg];
                let qi = &vtx2xyz[i_seg + 1];
                for j_seg in poly2vtx[j_poly]..poly2vtx[j_poly + 1] - 1 {
                    let pj = &vtx2xyz[j_seg];
                    let qj = &vtx2xyz[j_seg + 1];
    ...
    usize: num_traits::AsPrimitive<T>,
{
    use num_traits::AsPrimitive;
    let ied: usize = r.as_();
    let ned = vtx2xyz.len() - 1;
    assert!(ied < ned);
    let p0 = &vtx2xyz[ied];
    let p1 = &vtx2xyz[ied + 1];
    let r0 = r - ied.as_();
    use del_geo_core::vec3::Vec3;
    ...
    T: num_traits::Float + std::ops::AddAssign + std::fmt::Debug,
{
    ...
    for i_vtx in 0..num_vtx - 1 {
        let j_vtx = i_vtx + 1;
        let elen = del_geo_core::edge3::length(&vtx2xyz[i_vtx], &vtx2xyz[j_vtx]);
        len += elen;
    }
    len
}
...
pub fn reduce(vtx2xyz: &[[f32; 3]], threshold: f32) -> Vec<[f32; 3]> {
    let num_vtx = vtx2xyz.len();
