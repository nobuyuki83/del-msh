    ...
    Real: num_traits::Float,
{
    ...
    for v in &mut vtx2nrm {
        del_geo_core::vec3::normalize_in_place(v);
    }
    vtx2nrm
}
    ...
    Real: num_traits::Float,
{
    ...
    assert_eq!(ovtx2nrm.len(), num_ovtx);
    ovtx2nrm.fill([Real::zero(); 3]);
    for node2vtx in tri2vtx.iter() {
        let (i0, i1, i2) = (node2vtx[0], node2vtx[1], node2vtx[2]);
        let p0 = &vtx2xyz[i0];
        let p1 = &vtx2xyz[i1];
        ...
        for i_vtx in node2vtx {
            ...
            ovtx2nrm[i_ovtx][0] = ovtx2nrm[i_ovtx][0] + un[0];
            ovtx2nrm[i_ovtx][1] = ovtx2nrm[i_ovtx][1] + un[1];
            ovtx2nrm[i_ovtx][2] = ovtx2nrm[i_ovtx][2] + un[2];
        }
    }
    for v in ovtx2nrm.iter_mut() {
        del_geo_core::vec3::normalize_in_place(v);
    }
}
...
pub fn vtx2curvature_gaussian<T>(tri2vtx: &[[usize; 3]], vtx2xyz: &[[T; 3]]) -> Vec<T>
where
    T: num_traits::Float + std::fmt::Debug + num_traits::FloatConst,
{
    ...
    for i_vtx in 0..num_vtx {
        ...
        for i_tri in &idx2tri[vtx2idx[i_vtx]..vtx2idx[i_vtx + 1]] {
            let i_node = crate::tri2vtx::find_node_tri(&tri2vtx[*i_tri], i_vtx);
            assert_eq!(tri2vtx[*i_tri][i_node], i_vtx);
            let i0_vtx = tri2vtx[*i_tri][(i_node + 2) % 3];
            let i1_vtx = tri2vtx[*i_tri][i_node];
            let i2_vtx = tri2vtx[*i_tri][(i_node + 1) % 3];
    ...
    Index: num_traits::PrimInt + AsPrimitive<usize>,
{
    ...
    (vtx2xyz[i0], vtx2xyz[i1], vtx2xyz[i2])
}
...
pub fn tri2area(tri2vtx: &[[usize; 3]], vtx2xyz: &[[f32; 3]]) -> Vec<f32> {
...
pub fn mean_edge_length(tri2vtx: &[[usize; 3]], vtx2xyz: &[[f32; 3]]) -> f32 {
    let num_tri = tri2vtx.len();
    let mut sum = 0_f32;
    for node2vtx in tri2vtx.iter() {
        let (i0, i1, i2) = (node2vtx[0], node2vtx[1], node2vtx[2]);
...
pub fn distance_to_point3(tri2vtx: &[[usize; 3]], vtx2xyz: &[[f32; 3]], q: &[f32; 3]) -> f32 {
    use del_geo_core::vec3::Vec3;
...
pub fn distance_to_points3(tri2vtx: &[[usize; 3]], vtx2xyz: &[[f32; 3]], hv2xyz: &[[f32; 3]]) -> f32 {
...
pub fn area<T>(tri2vtx: &[[usize; 3]], vtx2xyz: &[[T; 3]]) -> T
where
    T: num_traits::Float,
{
    let mut sum_area = T::zero();
...
pub fn cog_and_area(tri2vtx: &[[usize; 3]], vtx2xyz: &[[f32; 3]]) -> Option<([f32; 3], f32)> {
    use del_geo_core::vec3;
