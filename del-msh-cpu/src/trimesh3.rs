...
pub fn vtx2normal<Real>(tri2vtx: &[usize], vtx2xyz: &[[Real; 3]]) -> Vec<Real>
where
    Real: num_traits::Float,
{
    let mut vtx2nrm = vec![Real::zero(); vtx2xyz.len() * 3];
...
pub fn vtx2area<T>(tri2vtx: &[usize], vtx2xyz: &[[T; 3]]) -> Vec<T>
where
...
fn test_vtx2area() {
    ...
    let vtx2area = crate::trimesh3::vtx2area(&tri2vtx.as_flattened(), &vtx2xyz);
    let total_area: f64 = vtx2area.iter().sum();
    assert!((total_area - std::f64::consts::PI * 4.0).abs() < 1.0e-2);
}

pub fn vtx2curvature_gaussian<T>(tri2vtx: &[usize], vtx2xyz: &[[T; 3]]) -> Vec<T>
where
...
fn test_vtx2curvature_gaussian() {
    ...
    let vtx2curv = vtx2curvature_gaussian(&tri2vtx.as_flattened(), &vtx2xyz);
...
pub fn tri2area(tri2vtx: &[usize], vtx2xyz: &[[f32; 3]]) -> Vec<f32> {
    let num_tri = tri2vtx.len() / 3;
    let mut tri2area = Vec::<f32>::with_capacity(num_tri);
    for i_tri in 0..num_tri {
        let area = to_tri3(tri2vtx, vtx2xyz.as_flattened(), i_tri).area();
        tri2area.push(area);
    }
    tri2area
}
...
pub fn mean_edge_length(tri2vtx: &[usize], vtx2xyz: &[[f32; 3]]) -> f32 {
    let num_tri = tri2vtx.len() / 3;
    let mut sum = 0_f32;
    for node2vtx in tri2vtx.chunks(3) {
        let (i0, i1, i2) = (node2vtx[0], node2vtx[1], node2vtx[2]);
...
pub fn distance_to_point3(tri2vtx: &[usize], vtx2xyz: &[[f32; 3]], q: &[f32; 3]) -> f32 {
    use del_geo_core::vec3::Vec3;
...
pub fn area<T>(tri2vtx: &[usize], vtx2xyz: &[[T; 3]]) -> T
where
    T: num_traits::Float,
{
    let mut sum_area = T::zero();
    for i_tri in 0..tri2vtx.len() / 3 {
        sum_area = sum_area + to_tri3(tri2vtx, vtx2xyz.as_flattened(), i_tri).area();
    }
    sum_area
}

pub fn cog_and_area(tri2vtx: &[usize], vtx2xyz: &[[f32; 3]]) -> Option<([f32; 3], f32)> {
    use del_geo_core::vec3;
    let mut sum_area = 0f32;
    let mut sum_cg = [0f32; 3];
    for i_tri in 0..tri2vtx.len() / 3 {
        let tri = to_tri3(tri2vtx, vtx2xyz.as_flattened(), i_tri);
        let area = tri.area();
        sum_area += area;
