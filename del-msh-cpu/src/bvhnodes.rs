...
pub fn mark_child<INDEX>(bvhnodes: &[[INDEX; 3]], inode0: usize, vtx2flag: &mut [usize])
where
    INDEX: num_traits::PrimInt + num_traits::AsPrimitive<usize>,
{
    assert!(inode0 < bvhnodes.len());
    if bvhnodes[inode0][2] == INDEX::max_value() {
        // leaf
        let in0: usize = bvhnodes[inode0][1].as_();
        assert!(in0 < vtx2flag.len(), "{} {}", in0, vtx2flag.len());
        vtx2flag[in0] += 1;
        return;
    }
    let in0 = bvhnodes[inode0][1].as_();
    let in1 = bvhnodes[inode0][2].as_();
    mark_child(bvhnodes, in0, vtx2flag);
    mark_child(bvhnodes, in1, vtx2flag);
}

pub fn check_bvh_topology<INDEX>(bvhnodes: &[[INDEX; 3]], num_vtx: usize)
where
    INDEX: num_traits::PrimInt + num_traits::AsPrimitive<usize> + std::fmt::Debug,
{
    ...
    assert_eq!(vtx2cnt, vec!(1usize; num_vtx));
    {
        for i_vtx in 0..num_vtx {
            ...
            assert_eq!(bvhnodes[i_bvhnode][2], INDEX::max_value());
        }
    }
}
