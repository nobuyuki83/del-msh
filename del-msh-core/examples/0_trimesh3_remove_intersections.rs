fn make_toy_problem(fpath_start: &str, fpath_goal: &str) {
    let (tri2vtx, vtx2xyz) = {
        let (tri2vtx, mut vtx2xyz) =
            del_msh_core::trimesh3_primitive::capsule_yup(0.3, 1.0, 16, 8, 16);
    ...
    }
    {
        ...
        let tri2vtx1 = tri2vtx.as_slice().to_vec();
        del_msh_core::uniform_mesh::merge(&mut tri2vtx0, &mut vtx2xyz0, &tri2vtx1, &vtx2xyz1, 3);
        del_msh_core::io_obj::save_tri2vtx_vtx2xyz(fpath_goal, &tri2vtx0, &vtx2xyz0, 3).unwrap()
    }
}
...
fn main() -> anyhow::Result<()> {
    ...
    let (tri2vtx, vtx2xyz_start) =
        del_msh_core::io_obj::load_tri_mesh::<_, usize, f64>(obj_file_path_start, None)?;
    /*
    {
        dbg!(tri2vtx.len()/3);
    ...
    let (_tri2vtx, vtx2xyz_goal) =
        del_msh_core::io_obj::load_tri_mesh::<_, usize, f64>(obj_file_path_goal, None)?;
    assert_eq!(tri2vtx, _tri2vtx);
    let vtx2xyz_out =
        del_msh_core::trimesh3_move_avoid_intersection::match_vtx2xyz_while_avoid_collision(
            &tri2vtx,
            &vtx2xyz_start,
            &vtx2xyz_goal,
