...
use pyo3::{pyfunction, types::PyModule, PyResult, Python, PyObject, ToPyObject, wrap_pyfunction, Bound};
...
pub fn load_wavefront_obj(
    py: Python,
    path_file: String) -> (Bound<PyArray2<f32>>, Bound<PyArray2<f32>>, Bound<PyArray2<f32>>,
                           Bound<PyArray1<usize>>,
                           Bound<PyArray1<usize>>, Bound<PyArray1<usize>>, Bound<PyArray1<usize>>,
                           Bound<PyArray1<usize>>, PyObject,
                           Bound<PyArray1<usize>>, PyObject, PyObject) {
    ...
    (
        numpy::ndarray::Array2::from_shape_vec(
            (obj.vtx2xyz.len()/3,3), obj.vtx2xyz).unwrap().into_pyarray_bound(py),
        numpy::ndarray::Array2::from_shape_vec(
            (obj.vtx2uv.len()/2,2), obj.vtx2uv).unwrap().into_pyarray_bound(py),
        numpy::ndarray::Array2::from_shape_vec(
            (obj.vtx2nrm.len()/3,3), obj.vtx2nrm).unwrap().into_pyarray_bound(py),
...
pub fn load_wavefront_obj_as_triangle_mesh(
    py: Python,
    path_file: String) -> (Bound<PyArray2<usize>>, Bound<PyArray2<f32>>)
{
    let Ok((tri2vtx, vtx2xyz))
        = del_msh::io_obj::load_tri_mesh(path_file, None) else { todo!() };
    (
        numpy::ndarray::Array2::from_shape_vec(
