...
fn torus_meshtri3(
    ...
    let (tri2vtx, vtx2xyz) =
        del_msh::trimesh3_primitive::torus_yup::<usize, f64>(radius, radius_tube, nlg, nlt);
...
fn capsule_meshtri3(
    ...
    let (tri_vtx, vtx_xyz) = del_msh::trimesh3_primitive::capsule_yup::<f64>(r, l, nc, nr, nl);
...
fn sphere_meshtri3(
    ...
    let (tri2vtx, vtx2xyz) = del_msh::trimesh3_primitive::sphere_yup(r, nr, nl);
    let v = numpy::ndarray::Array2::from_shape_vec((vtx2xyz.len() / 3, 3), vtx2xyz).unwrap();
    let f = numpy::ndarray::Array2::from_shape_vec((tri2vtx.len() / 3, 3), tri2vtx).unwrap();
    (f.into_pyarray_bound(py), v.into_pyarray_bound(py))
}
...
fn trimesh3_hemisphere_zup(
    ...
    let (tri2vtx, vtx2xyz) = del_msh::trimesh3_primitive::hemisphere_zup(r, nr, nl);
    (
