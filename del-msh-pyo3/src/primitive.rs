...
use numpy::{IntoPyArray,
            PyArray2};
use pyo3::{Python, pyfunction, types::PyModule, PyResult, wrap_pyfunction, Bound};
...
fn torus_meshtri3(
    ...
    nlg: usize, nlt: usize) -> (Bound<PyArray2<usize>>, Bound<PyArray2<f64>>) {
    let (tri2vtx, vtx2xyz)
        = del_msh::trimesh3_primitive::torus_yup::<usize, f64>(
...
fn capsule_meshtri3(
    py: Python,
    r: f64, l: f64,
    nc: usize, nr: usize, nl: usize) -> (Bound<PyArray2<usize>>, Bound<PyArray2<f64>>) {
    let ( tri_vtx, vtx_xyz) = del_msh::trimesh3_primitive::capsule_yup::<f64>(
        r, l, nc, nr, nl);
    let v = numpy::ndarray::Array2::from_shape_vec(
        (vtx_xyz.len()/3,3), Vec::from(vtx_xyz.as_slice())).unwrap();
    let f = numpy::ndarray::Array2::from_shape_vec(
        (tri_vtx.len()/3,3), Vec::from(tri_vtx.as_slice())).unwrap();
    (f.into_pyarray_bound(py), v.into_pyarray_bound(py))
}
    ...
    -> (Bound<PyArray2<usize>>, Bound<PyArray2<f64>>)
{
    let (tri2vtx, vtx2xyz) = if is_closed_end {
        del_msh::trimesh3_primitive::cylinder_closed_end_yup::<f64>(
            radius, length, ndiv_circumference, ndiv_length, is_center)
    } else {
        del_msh::trimesh3_primitive::cylinder_open_end_yup::<f64>(
            ndiv_circumference, ndiv_length, radius, length, is_center)
    };
    let v = numpy::ndarray::Array2::from_shape_vec(
        (vtx2xyz.len()/3, 3), Vec::from(vtx2xyz.as_slice())).unwrap();
    let f = numpy::ndarray::Array2::from_shape_vec(
        (tri2vtx.len()/3, 3), Vec::from(tri2vtx.as_slice())).unwrap();
    (f.into_pyarray_bound(py), v.into_pyarray_bound(py))
}
...
fn sphere_meshtri3(
    py: Python,
    r: f32,
    nr: usize, nl: usize) -> (Bound<PyArray2<usize>>, Bound<PyArray2<f32>>) {
...
fn trimesh3_hemisphere_zup(
    ...
    (
        numpy::ndarray::Array2::from_shape_vec(
            (tri2vtx.len()/3, 3), tri2vtx).unwrap().into_pyarray_bound(py),
        numpy::ndarray::Array2::from_shape_vec(
        (vtx2xyz.len()/3, 3), vtx2xyz).unwrap().into_pyarray_bound(py)
    )
}
