...
fn intersection_trimesh3<'a>(
    ...
    -> (Bound<'a, numpy::PyArray3<f32>>, Bound<'a, numpy::PyArray2<usize>>)
{
    ...
    (
        numpy::ndarray::Array3::from_shape_vec(
            (edge2node2xyz.len() / 6, 2, 3), edge2node2xyz)
            .unwrap().to_pyarray_bound(_py),
        numpy::ndarray::Array2::from_shape_vec(
            (edge2tri.len() / 2, 2), edge2tri)
            .unwrap().to_pyarray_bound(_py)
    )
}
...
fn ccd_intersection_time<'a>(
    ...
    aabbs: numpy::PyReadonlyArray2<'a, f32>,
    roots: Vec<usize>)
    -> (Bound<'a, numpy::PyArray2<usize>>, Bound<'a, numpy::PyArray1<f32>>)
{
    ...
    (
        numpy::ndarray::Array2::from_shape_vec(
            ...
            intersecting_pair).unwrap().to_pyarray_bound(_py),
...
fn contacting_pair<'a>(
    ...
    edge2vtx: numpy::PyReadonlyArray2<'a, usize>,
    threshold: f32)
    -> (Bound<'a, numpy::PyArray2<usize>>, Bound<'a, numpy::PyArray2<f32>>)
{
    let tri2vtx = tri2vtx.as_slice().unwrap();
    let vtx2xyz = vtx2xyz.as_slice().unwrap();
    let edge2vtx = edge2vtx.as_slice().unwrap();
    ...
    (
        numpy::ndarray::Array2::from_shape_vec(
            (contacting_pair.len() / 3, 3), contacting_pair)
            .unwrap().to_pyarray_bound(_py),
        numpy::ndarray::Array2::from_shape_vec(
            (contacting_coord.len() / 4, 4), contacting_coord)
            .unwrap().to_pyarray_bound(_py)
    )
}