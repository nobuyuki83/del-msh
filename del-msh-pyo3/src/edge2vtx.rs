...
fn edge2vtx_uniform_mesh<'a>(
    ...
    num_vtx: usize) -> Bound<'a, numpy::PyArray2<usize>> {
    ...
    numpy::ndarray::Array2::from_shape_vec(
        (mshline.len() / 2, 2), mshline).unwrap().into_pyarray_bound(py)
}
...
fn boundaryedge2vtx_triangle_mesh<'a>(
    ...
    num_vtx: usize)
    -> (Bound<'a, numpy::PyArray2<usize>>, Bound<'a, numpy::PyArray2<usize>>)
{
    // let num_node = tri2vtx.shape()[1];
    // let num_tri = tri2vtx.shape()[0];
    let tri2vtx = tri2vtx.as_slice().unwrap();
    let (bedge2vtx, tri2tri)
        = del_msh::trimesh_topology::boundaryedge2vtx(tri2vtx, num_vtx);
    (
        numpy::ndarray::Array2::from_shape_vec(
            (bedge2vtx.len() / 2, 2), bedge2vtx).unwrap().into_pyarray_bound(py),
        numpy::ndarray::Array2::from_shape_vec(
            (tri2tri.len() / 3, 3), tri2tri).unwrap().into_pyarray_bound(py)
    )
}