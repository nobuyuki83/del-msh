...
def vtx2vtx(
    tri2vtx: numpy.typing.NDArray, num_vtx: int, is_self=False
...
def vtx2area(tri2vtx: numpy.typing.NDArray, vtx2xyz: numpy.typing.NDArray):
...
def edge2vtx(tri2vtx: numpy.typing.NDArray, num_vtx: int) -> numpy.typing.NDArray:
    """
...
def boundaryedge2vtx(
    tri2vtx: numpy.typing.NDArray, num_vtx: int
) -> (numpy.typing.NDArray, numpy.typing.NDArray):
    assert len(tri2vtx.shape) == 2
    assert tri2vtx.shape[1] == 3
    assert not numpy.isfortran(tri2vtx)
...
def tri2tri(tri2vtx: numpy.typing.NDArray, num_vtx: int) -> numpy.typing.NDArray:
...
def tri2distance(idx_tri: int, tri2tri: numpy.typing.NDArray) -> numpy.typing.NDArray:
...
def tri2area(
    ...
    :return:
    """
    assert vtx2xyz.shape[1] == 2 or vtx2xyz.shape[1] == 3, (
...
def tri2circumcenter(tri2vtx: numpy.typing.NDArray, vtx2xyz: numpy.typing.NDArray):
...
def torus(
    major_radius=1.0, minor_radius=0.4, ndiv_major_radius=32, ndiv_minor_radius=32
) -> (numpy.typing.NDArray, numpy.typing.NDArray):
    from .del_msh_numpy import trimesh3_primitive_torus_zup

    return trimesh3_primitive_torus_zup(
        major_radius, minor_radius, ndiv_major_radius, ndiv_minor_radius
    )
...
def capsule(
    ...
    return trimesh3_primitive_capsule_yup(
        radius, height, ndiv_theta, ndiv_longtitude, ndiv_height
    )
...
def cylinder(
    ...
    return trimesh3_primitive_cylinder_yup(
        radius, height, ndiv_circumference, ndiv_height, is_closed_end, is_center
    )


def sphere(
    radius: float = 1.0, ndiv_latitude: int = 32, ndiv_longtitude: int = 32
) -> (numpy.typing.NDArray, numpy.typing.NDArray):
...
def hemisphere(
    radius: float = 1.0, ndiv_longtitude: int = 32, ndiv_latitude: int = 32
) -> (numpy.typing.NDArray, numpy.typing.NDArray):
...
def load_wavefront_obj(
    path_file: str, is_centerize=False, normalized_size: typing.Optional[float] = None
):
...
def sample_many(
    tri2vtx: numpy.typing.NDArray, vtx2xy, num_sample: int
) -> numpy.ndarray:
    import random

    tri2area_ = tri2area(tri2vtx, vtx2xy)
    cumsum_area = numpy.cumsum(
        numpy.append(numpy.zeros(1, dtype=numpy.float32), tri2area_)
    )
...
def bvhnodes_tri(
    tri2vtx: numpy.typing.NDArray, vtx2xyz: numpy.typing.NDArray, is_morton=True
) -> (numpy.typing.NDArray, numpy.typing.NDArray):
    """
    ...
    if is_morton:
        ...
        del_msh_numpy.fit_into_unit_cube(
            tri2center
...
def bvhnodes_vtxedgetri(
    ...
    return bvhnodes, [
        0,
        bvhnodes_vtx.shape[0],
        bvhnodes_vtx.shape[0] + bvhnodes_edge.shape[0],
    ]
...
def aabb_vtxedgetri(
    ...
    aabbs = aabb_uniform_mesh(
        edge2vtx, vtx2xyz0, bvhnodes, aabbs=aabbs, root=roots[0], vtx2xyz1=vtx2xyz1
    )
    # triangle
    aabbs = aabb_uniform_mesh(
        tri2vtx, vtx2xyz0, bvhnodes, aabbs=aabbs, root=roots[0], vtx2xyz1=vtx2xyz1
    )
    return aabbs
...
def ccd_intersection_time(
    ...
    if bvhnodes is not None:
        ...
        return ccd_intersection_time(
            edge2vtx, tri2vtx, vtx2xyz0, vtx2xyz1, bvhnodes, aabbs, roots
        )
    else:
        return ccd_intersection_time(
            edge2vtx,
...
def first_intersection_ray(
    src, dir, tri2vtx: numpy.typing.NDArray, vtx2xyz: numpy.typing.NDArray
...
def self_intersection(
    ...
    if vtx2xyz.shape[1] == 3:
        ...
        if bvhnodes is None:
            ...
            return intersection_trimesh3(
                tri2vtx, vtx2xyz, bvhnodes, aabbs, i_bvhnode_root
            )
