...
def direct_manipulation_delta(shape2pos, markers: dict[int, pyrr.Matrix44]):
    ...
    for vtx_marker in markers.keys():
        ...
        pos_0 = mvp44.dot(numpy.append(pos_0, 1.0))[
            0:2
        ]  # screen position of the marker
        for idx_weight in range(num_dweight):
            idx_shape = idx_weight + 1
            pos_i = (
                shape2pos[idx_shape].reshape(-1, 3)[vtx_marker].copy()
            )  # 3D position of the marker
            pos_i = mvp44.dot(numpy.append(pos_i, 1.0))[
                0:2
            ]  # screen position of the marker
    ...
    G = numpy.vstack([numpy.eye(num_dweight), -numpy.eye(num_dweight)]).astype(
        numpy.double
    )
    h = numpy.vstack(
        [numpy.ones((num_dweight, 1)), numpy.zeros((num_dweight, 1))]
    ).astype(numpy.double)
    sol = cvxopt.solvers.qp(
        P=matrix(P), q=matrix(q), A=matrix(A), b=matrix(b), G=matrix(G), h=matrix(h)
    )
...
def direct_manipulation_absolute(shape2pos, markers):
    ...
    b = numpy.array([1.0]).reshape(1, 1)
    G = numpy.vstack([numpy.eye(num_shape), -numpy.eye(num_shape)]).astype(numpy.double)
    h = numpy.vstack([numpy.ones((num_shape, 1)), numpy.zeros((num_shape, 1))]).astype(
        numpy.double
    )
    sol = cvxopt.solvers.qp(
        P=matrix(P), q=matrix(q), A=matrix(A), b=matrix(b), G=matrix(G), h=matrix(h)
