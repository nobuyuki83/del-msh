...
class WavefrontObj:
    ...
    def centerize(self):
        self.vtxxyz2xyz[:] -= (
            self.vtxxyz2xyz.max(axis=0) + self.vtxxyz2xyz.min(axis=0)
        ) * 0.5

    def normalize_size(self, scale=1.0):
        self.vtxxyz2xyz *= (
            scale / (self.vtxxyz2xyz.max(axis=0) - self.vtxxyz2xyz.min(axis=0)).max()
        )

    def edge2vtxxyz(self):
        from .del_msh import edge2vtx_polygon_mesh

        return edge2vtx_polygon_mesh(
            self.elem2idx, self.idx2vtxxyz, self.vtxxyz2xyz.shape[0]
        )
    ...
    def triangle_mesh_with_uv(self):
        from .del_msh import (
            triangles_from_polygon_mesh,
            unify_two_indices_of_triangle_mesh,
        )
        ...
        tri2uni, uni2vtxxyz, uni2vtxuv = unify_two_indices_of_triangle_mesh(
            tri2vtxxyz, tri2vtxuv
        )
...
def load(
    file_path: str, is_centerize=False, normalized_size: typing.Optional[float] = None
):
