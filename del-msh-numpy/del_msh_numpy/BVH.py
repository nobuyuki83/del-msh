...
def edges_of_aabb(aabb: numpy.typing.NDArray) -> numpy.typing.NDArray:
    ...
    if num_dim == 3:
        ...
        edge2node2xyz[:, 0, 0, :] = edge2node2xyz[:, 3, 1, :] = edge2node2xyz[
            :, 8, 0, :
        ] = aabb[:, [0, 1, 2]]
        edge2node2xyz[:, 0, 1, :] = edge2node2xyz[:, 1, 0, :] = edge2node2xyz[
            :, 9, 0, :
        ] = aabb[:, [3, 1, 2]]
        edge2node2xyz[:, 2, 1, :] = edge2node2xyz[:, 3, 0, :] = edge2node2xyz[
            :, 10, 0, :
        ] = aabb[:, [0, 4, 2]]
        edge2node2xyz[:, 1, 1, :] = edge2node2xyz[:, 2, 0, :] = edge2node2xyz[
            :, 11, 0, :
        ] = aabb[:, [3, 4, 2]]
        edge2node2xyz[:, 4, 0, :] = edge2node2xyz[:, 7, 1, :] = edge2node2xyz[
            :, 8, 1, :
        ] = aabb[:, [0, 1, 5]]
        edge2node2xyz[:, 4, 1, :] = edge2node2xyz[:, 5, 0, :] = edge2node2xyz[
            :, 9, 1, :
        ] = aabb[:, [3, 1, 5]]
        edge2node2xyz[:, 6, 1, :] = edge2node2xyz[:, 7, 0, :] = edge2node2xyz[
            :, 10, 1, :
        ] = aabb[:, [0, 4, 5]]
        edge2node2xyz[:, 5, 1, :] = edge2node2xyz[:, 6, 0, :] = edge2node2xyz[
            :, 11, 1, :
        ] = aabb[:, [3, 4, 5]]
...
def aabb_uniform_mesh(
    ...
    if vtx2xyz0.dtype == numpy.float32:
        ...
        build_bvh_geometry_aabb_uniformmesh_f32(
            aabbs, bvhnodes, elem2vtx, vtx2xyz0, root, vtx2xyz1
        )
