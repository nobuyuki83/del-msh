...
where T: Copy + nalgebra::RealField
{
    ...
    let icnt0 = if i0 == j0 || i0 == j1 || i0 == j2 { 1 } else { 0 };
    ...
    if icnt0 + icnt1 + icnt2 == 0 {
        ...
        let jcnt0 = if j0 == i0 || j0 == i1 || j0 == i2 { 1 } else { 0 };
        let jcnt1 = if j1 == i0 || j1 == i1 || j1 == i2 { 1 } else { 0 };
        let jcnt2 = if j2 == i0 || j2 == i1 || j2 == i2 { 1 } else { 0 };
        assert_eq!(jcnt0+jcnt1+jcnt2,1);
        let js = if jcnt0 == 1 { 0 } else if jcnt1 == 1 { 1 } else { 2 };
    ...
    where T: nalgebra::RealField + Copy
{
    ...
    if !is_leaf0 && !is_leaf1 {
        ...
        let i_tri = ichild0_0;
        let j_tri = ichild1_0;
        if let Some((p0, p1)) = intersection_of_two_triangles_in_mesh(tri2vtx, vtx2xyz, i_tri, j_tri) {
            let itp = IntersectingPair { i_tri, j_tri, p0, p1, };
            pairs.push(itp);
        } else { return; }
    }
}
...
pub fn search_brute_force<T>(
    tri2vtx: &[usize],
    vtx2xyz: &[T]) -> Vec<IntersectingPair<T>>
