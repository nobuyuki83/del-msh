...
pub fn find_adjacent_edge_index(
    ...
    if tri2vtx[it1 * 3 + 1] == iv1 && tri2vtx[it1 * 3 + 2] == iv0 { return 0; }
    if tri2vtx[it1 * 3 + 2] == iv1 && tri2vtx[it1 * 3 + 0] == iv0 { return 1; }
    if tri2vtx[it1 * 3 + 0] == iv1 && tri2vtx[it1 * 3 + 1] == iv0 { return 2; }
    panic!();
}
    ...
    vtx2tri: &mut [usize]) -> bool
{
    ...
    let old_v: [usize; 3] = tri2vtx[idx_tri_insert * 3..idx_tri_insert * 3 + 3].try_into().unwrap();
    let old_s: [usize; 3] = tri2tri[idx_tri_insert * 3..idx_tri_insert * 3 + 3].try_into().unwrap();
    ...
    vtx2tri: &mut [usize]) -> bool
{
    ...
    let ied_adj = find_adjacent_edge_index(
        tri2vtx[idx_tri_insert * 3..idx_tri_insert * 3 + 3].try_into().unwrap(),
        tri2tri[idx_tri_insert * 3..idx_tri_insert * 3 + 3].try_into().unwrap(),
        idx_triedge_insert, tri2vtx);
    ...
    let old_a_v: [usize; 3] = tri2vtx[idx_tri_insert * 3..idx_tri_insert * 3 + 3].try_into().unwrap();
    let old_a_s: [usize; 3] = tri2tri[idx_tri_insert * 3..idx_tri_insert * 3 + 3].try_into().unwrap();
    ...
    tri2vtx[itri0 * 3..itri0 * 3 + 3].copy_from_slice(&[idx_vtx_insert, old_a_v[ino_a2], old_a_v[ino_a0]]);
    ...
    tri2vtx[itri1 * 3..itri1 * 3 + 3].copy_from_slice(&[idx_vtx_insert, old_a_v[ino_a0], old_a_v[ino_a1]]);
    ...
    tri2vtx[itri2 * 3..itri2 * 3 + 3].copy_from_slice(&[idx_vtx_insert, old_b_v[ino_b2], old_b_v[ino_b0]]);
    ...
    tri2vtx[itri3 * 3..itri3 * 3 + 3].copy_from_slice(&[idx_vtx_insert, old_b_v[ino_b0], old_b_v[ino_b1]]);
...
pub fn find_node(
    ...
    tri2vtx: &[usize],
    i_tri: usize) -> usize {
    if tri2vtx[i_tri * 3 + 0] == i_vtx { return 0; }
    if tri2vtx[i_tri * 3 + 1] == i_vtx { return 1; }
    if tri2vtx[i_tri * 3 + 2] == i_vtx { return 2; }
    usize::MAX
}
...
pub fn flip_edge(
    ...
    if tri2tri[itri_a * 3 + ied0] == usize::MAX { return false; }
    ...
    tri2vtx[itri_a * 3..itri_a * 3 + 3].copy_from_slice(&[old_a_v[no_a1], old_b_v[no_b0], old_a_v[no_a0]]);
    ...
    tri2vtx[itri_b * 3..itri_b * 3 + 3].copy_from_slice(&[old_b_v[no_b1], old_a_v[no_a0], old_b_v[no_b0]]);
...
pub fn move_ccw(
    ...
    if tri2tri[*itri_cur * 3 + inotri1] == itri_adj { return false; }
    ...
    assert_eq!(tri2vtx[*itri_cur * 3 + *inotri_cur], tri2vtx[itri_nex * 3 + inotri_nex]);
    *itri_cur = itri_nex;
...
pub fn move_cw(
    ...
    let ino2 = find_adjacent_edge_index(
        tri_vtx[*itri_cur * 3..*itri_cur * 3 + 3].try_into().unwrap(),
        tri2tri[*itri_cur * 3..*itri_cur * 3 + 3].try_into().unwrap(),
        inotri1, tri_vtx);
    let inotri_nex = (ino2 + 2) % 3;
    assert_eq!(tri_vtx[*itri_cur * 3 + *inotri_cur], tri_vtx[itri_nex * 3 + inotri_nex]);
    *itri_cur = itri_nex;
    *inotri_cur = inotri_nex;
    true
}
...
pub fn find_edge_by_looking_around_point(
    ...
    tri2vtx: &[usize],
    tri2tri: &[usize],
    vtx2tri: &[usize]) -> Option<(usize, usize, usize)>
{
    ...
    loop { // search counter clock-wise
        ...
        if itc == vtx2tri[ipo0] {  // end if it goes around
            return None;
        }
        let inotri2 = (inc + 1) % 3;
        if tri2vtx[itc*3+inotri2] == ipo1 {
...
pub fn find_edge_by_looking_all_triangles(
    ipo0: usize,
    ipo1: usize,
    tri2vtx: &[usize]) -> Option<(usize, usize)>
{
...
pub fn delete_tri_flag(
    ...
    tri2tri0: &[usize],
    tri2flag0: &[i32],
    flag: i32) -> (Vec<usize>, Vec<usize>, Vec<i32>)
{
