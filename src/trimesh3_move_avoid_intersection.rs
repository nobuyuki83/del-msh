    ...
    num_iter: usize) -> Vec<f64>
{
    ...
    let edge2vtx = crate::edge2vtx::from_triangle_mesh(
        tri2vtx, vtx2xyz_start.len() / 3);
    ...
    for _itr_outer in 0..num_iter {
        ...
        let (w0, dw0) = wdw(&edge2vtx, tri2vtx, &vtx2xyz, vtx2xyz_goal, dist0, k_contact, k_diff);
        let step: Vec<_> = dw0.iter().map(|r| r * -alpha).collect();
        let vtx2xyz_dist: Vec<f64> = vtx2xyz.iter().zip(step.iter())
            .map(|(v, r)| v + r).collect();
        //
        let time_max = {
        ...
        let mut coeff = 0.9;
        for _itr in 0..10 {
            let vtx2xyz_cand: Vec<_> = vtx2xyz.iter().zip(step.iter())
                .map(|(v, r)| v + r * time_max * coeff).collect();
            {
                let tripairs = crate::trimesh3_intersection::search_brute_force(
            ...
            dbg!(w0,w);
            if w < w0 {
                vtx2xyz.iter_mut().zip(vtx2xyz_cand.iter()).for_each(|(v, &c)| *v = c);
                break;
            }
            coeff *= 0.5;
