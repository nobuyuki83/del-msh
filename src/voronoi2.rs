...
impl Cell {
    ...
    fn new_from_polyloop2(vtx2xy_in: &[nalgebra::Vector2::<f32>]) -> Self {
        ...
        Cell { vtx2xy: vtx2xy_in.to_owned(), vtx2info }
    }
    ...
    -> Option<Cell>
{
    ...
    let (i_vtx0, is_new0) = {
        let Some(i_vtxnew_start)
            = vtxnew2isvisisted.iter().position(|is_visited| !is_visited)
            else { return None; };
        (i_vtxnew_start, true)
    };
    ...
    loop {
        ...
        if i_vtx == i_vtx0 && is_new == is_new0 { break; }
    }
    Some(Cell{vtx2xy: vtx2xy_new, vtx2info:vtx2info_new})
}
    ...
    -> Vec<Cell>
{
    ...
    if vtxnews.is_empty() { // no intersection
        return if is_inside { vec!(cell.clone()) } else { vec!() }
    }
    ...
    loop {
        let c0 = hoge(
            &cell.vtx2xy, &cell.vtx2info,
            &vtxnews, &vtx2vtxnew, &mut vtxnew2isvisisted);
        let Some(cell) = c0 else { break; };
        cells.push(cell);
    }
    cells
}
    ...
    for (i_site, pos_i) in site2xy.iter().enumerate() {
        ...
        for (j_site, pos_j) in site2xy.iter().enumerate() {
            ...
            for cell_in in cell_stack {
                let cells = cut_polygon_by_line(
                    &cell_in,
                    &line_s, &line_n, i_site, j_site);
