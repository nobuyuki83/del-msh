    ...
    vtx2xyz: &[f32])
{
    ...
    vtx2xyz.chunks(3).zip(vtx2morton.iter_mut()).for_each(
        |(xyz, m)| *m = morton_code(xyz[0], xyz[1], xyz[2]));
    idx2vtx.iter_mut().enumerate().for_each(|(iv, idx)| *idx = iv);
...
fn morton_code_determine_range(
    idx2morton: &[u32],
    idx1: usize) -> (usize, usize)
{
    ...
    loop {
        ...
        if jmc < 0 || jmc >= idx2morton.len() as i64 { break; }
        if delta(idx1, jmc.try_into().unwrap(), idx2morton) <= delta_min { break; }
        lmax *= 2;
    }
    ...
    if idx1 <= j { (idx1, j) } else { (j, idx1) }
}

#[allow(dead_code)]
fn check_morton_code_range_split(
    idx2morton: &[u32])
{
    assert!(!idx2morton.is_empty());
    for ini in 0..idx2morton.len() - 1 {
        let range = morton_code_determine_range(
            idx2morton, ini);
        let isplit = morton_code_find_split(idx2morton, range.0, range.1);
        let range_a = morton_code_determine_range(
            idx2morton, isplit);
        let range_b = morton_code_determine_range(
            idx2morton, isplit + 1);
        assert_eq!(range.0, range_a.0);
        assert_eq!(range.1, range_b.1);
        let last1 = if isplit == range.0 { isplit } else { range_a.1 };
        let first1 = if isplit + 1 == range.1 { isplit + 1 } else { range_b.0};
        assert_eq!(last1 + 1, first1);
    }
}


fn morton_code_find_split(
    idx2morton: &[u32],
    i_mc_start: usize,
    i_mc_end: usize) -> usize
{
    if i_mc_start == i_mc_end { return usize::MAX; }
    ...
    if nbitcommon0 == 32 { return i_mc_start; } // sizeof(std::uint32_t)*8
